    ...
    ;(async () => {
      try {
        ...
        if (!res?.alreadyAccepted) {
          toast.success(
            '🎉 ตอบรับสำเร็จ! ดูตารางเรียน แจ้งลา และดูผลการเรียนของบุตรหลานได้เลย',
            { duration: 6000 }
          )
          // ยังไม่มีข้อมูลติดต่อของผู้รับคนนี้ → พาไปกรอกที่หน้าโปรไฟล์
          // (ฟอร์มเปิดรออยู่แล้วเมื่อยังไม่มีเบอร์) ทางโรงเรียนจะได้ติดต่อกลับได้
          setTimeout(() => router.push('/liff/profile'), 1200)
        }
      } catch (e: any) {
