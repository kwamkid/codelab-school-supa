...
export async function POST(request: NextRequest) {
  try {
    ...
    const pagesRes = await fetch(
      `https://graph.facebook.com/v19.0/me/accounts?fields=id,name,access_token,picture{url}&limit=100&access_token=${longLivedUserToken}`
    );
    ...
    if (!pagesData.data || pagesData.data.length === 0) {
      return NextResponse.json(
        { error: 'ไม่พบเพจ Facebook ที่คุณมีสิทธิ์จัดการ' },
        { status: 404 }
      );
    }
