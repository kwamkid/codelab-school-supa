    ...
    for (const student of activeStudents) {
      ...
      for (const enrollment of activeEnrollments) {
        try {
          ...
          // Get schedules within date range (from Supabase)
          const allSchedules = await getClassSchedules(classData.id);
          const schedules = allSchedules.filter(
            s => s.sessionDate >= start && s.sessionDate <= end
          );
          console.log(`[getParentScheduleEvents] Found ${schedules.length} schedules for class ${classData.name}`);
          ...
          for (const schedule of schedules) {
            ...
            console.log(`[getParentScheduleEvents] Schedule ${schedule.id}, session ${schedule.sessionNumber}, hasMakeup: ${hasMakeupRequest}`);
