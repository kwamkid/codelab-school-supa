// lib/supabase/services/line-api.ts

import { Client } from '@line/bot-sdk'
import { getLineSettings } from './line-settings'

// Test LINE Messaging API connection
export async function testMessagingAPI(channelAccessToken: string): Promise<{
  success: boolean
  message: string
  data?: any
}> {
...
export async function verifyWebhookEndpoint(webhookUrl: string): Promise<{
  success: boolean
  message: string
}> {
  try {
...
export async function sendMulticastMessage(
  ...
  try {
    ...
    return {
      success: true,
      message: `ส่งข้อความถึง ${userIds.length} คนสำเร็จ`
    }
  } catch (error: any) {
    ...
    return {
      success: false,
      message: error.message || 'ไม่สามารถส่งข้อความได้'
    }
  }
}
