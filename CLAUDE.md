...
- **Engineering Notebook — 2 ลิงก์ต่อทีม**: `vex.teams.notebook_url` = ฉบับกำลังทำ (Canva/Slides), `notebook_submit_url` = ฉบับส่งจริง (PDF บน Drive) — migrations `20260818_vex_team_notebook_url` + `_submit_url`. กรอกในฟอร์มสร้าง/แก้ทีม (zod `.url()`), การ์ดทีมมี 2 ลิงก์เปิดแท็บใหม่ (น้ำเงิน=กำลังทำ, เขียว=ฉบับส่ง) ทีมที่ยังไม่ใส่ขึ้นข้อความเทา "ยังไม่มีเล่มกำลังทำ / ยังไม่ส่ง PDF". **หน้า `/vexteam` (ทีม) ครูเข้าดูได้แล้ว (view-only)** — `canManage` ซ่อนปุ่มสร้าง/แก้/ลบ/เพิ่ม-ลบสมาชิก/ลิงก์ทีม.
