...
- **ลิงก์ทีมแบบเก่า (`/team/...`) พาเข้าแอปผู้ปกครองแล้ว (30 ส.ค. 26)**: ถ้าคนเปิดเป็นผู้ปกครองที่ลงทะเบียนแล้ว (รวมผู้ปกครองร่วม) → `components/vex/portal-redirect.tsx` พาไป `/liff/team`; คนที่ยังไม่ลงทะเบียนยังใช้หน้าเดิมได้. **`getVexParentByLineId` ต้อง fallback ไป `parent_line_recipients` ด้วย** — เดิมเช็คแต่ `parents.line_user_id` ผู้ปกครองร่วมเปิดลิงก์ทีมแล้วโดนไล่ให้ "ลงทะเบียนใหม่" ทุกครั้ง → กดสมัครซ้ำ = **ได้นักเรียนซ้ำเพิ่มทุกรอบ** (บ้านหนึ่งได้ลูกซ้ำ 5 แถวใน 1 เดือน). `registerParentWithStudent`/`createStudentForParent` มี `findExistingStudent` (ชื่อเล่น+วันเกิด) กันซ้ำแล้ว.
...
- **เพิ่มกิจกรรมลงปฏิทินเครื่อง**: ไม่มี URL scheme กลางที่พารายละเอียดไปได้ (iOS `calshow:` เปิดได้แค่แอปเปล่า ๆ) → ต้องเสิร์ฟไฟล์ `text/calendar`. `GET /api/calendar/vex-event?id=` คืน .ics (all-day, DTEND **exclusive +1 วัน**, VALARM เตือนล่วงหน้า 1 วัน, พับบรรทัดตาม RFC เพราะชื่อไทย 3 ไบต์/ตัวเกิน 75 octet ง่าย) — ไม่ต้อง auth เพราะ OS เป็นคนโหลด. ฝั่ง UI: iOS ใช้ .ics, ที่เหลือใช้ `googleCalendarUrl()` (`lib/utils.ts`) และ **ต้องเปิดผ่าน `liff.openWindow({external:true})`** เพราะ webview ของ LINE ดาวน์โหลดไฟล์ไม่ได้.
- **แท็บล่างใช้ `components/liff/bottom-tab-bar.tsx` (`<BottomTabBar tabs>`) ร่วมกัน 2 ที่** — แอปผู้ปกครอง (`components/liff/bottom-nav.tsx`) และ /team (`app/team/team-bottom-nav.tsx`); ไฟล์ทั้งสองเหลือแค่รายการแท็บ. แท็บ "ทีม" โผล่เฉพาะบ้านที่มีลูกอยู่ทีม — จำคำตอบใน `localStorage['liff:has-team']` แล้วอ่านคืนใน `useLayoutEffect` (ก่อนวาด) ไม่งั้นจะเห็น 3 แท็บแล้วเด้งเป็น 4 ทุกครั้งที่เปิดแอป.
