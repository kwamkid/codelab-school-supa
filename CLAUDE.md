...
- `Button` size `sm` is `text-sm` (default size stays `text-base`). เจ้าของอ่านหน้าจอจริงแล้วบ่นบ่อยว่า **เล็กไป** — หน้าใหม่ใช้ default size + `text-base` ไว้ก่อน อย่าใส่ `size="sm"`/`text-sm` เพราะเห็นว่าดูกระชับ
