...
- **Engineering Notebook**: `vex.teams.notebook_url` (migration `20260818_vex_team_notebook_url`) — ลิงก์เดียวต่อทีม (Canva/Drive/ฯลฯ) กรอกในฟอร์มสร้าง/แก้ทีม (zod `.url()`), การ์ดทีมมีลิงก์เปิดใหม่แท็บ ทีมที่ยังไม่ใส่ขึ้น "ยังไม่มี Notebook" สีเทา. **หน้า `/vexteam` (ทีม) ครูเข้าดูได้แล้ว (view-only)** — `canManage` ซ่อนปุ่มสร้าง/แก้/ลบ/เพิ่ม-ลบสมาชิก/ลิงก์ทีม.
