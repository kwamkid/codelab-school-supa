...
- **ปฏิทินซ้อม = `components/vex/practice-calendar.tsx` ใช้ร่วมกัน 2 ที่** (/team ลิงก์ token + แท็บทีมในแอปผู้ปกครอง `/liff/team`): prop `kids` = เด็ก**ทั้งทีม** (ไว้โชว์ชื่อบนปฏิทิน/ตัวกรอง — ไม่งั้นของบ้านอื่นขึ้นเป็น `-`), `proposableKids` = เฉพาะคนที่เสนอซ้อมให้ได้ (แอปผู้ปกครองส่งลูกตัวเอง, /team ไม่ส่ง = ทั้งทีมตามเดิม). รายการใต้ปฏิทินแยกเป็นแท็บ รออนุมัติ/อนุมัติแล้ว/ไม่อนุมัติ (ค่าเริ่มต้นเปิดแท็บที่มีของรออนุมัติก่อน).
