-- Fix: get_weekly_timetable showed the ORIGINAL class teacher even after a
-- teacher change. Two change cases exist:
--   1. Permanent change (changeClassResources) → updates classes.teacher_id AND
--      sets class_schedules.actual_teacher_id on future sessions.
--   2. Per-session substitute (updateClassSchedule) → sets ONLY
--      class_schedules.actual_teacher_id; classes.teacher_id stays the same.
-- The old RPC joined teachers on classes.teacher_id only, so case 2 (and any
-- per-session nuance of case 1) never showed in the report.
--
-- Fix: drive regular classes off class_schedules for the actual sessions in the
-- week, resolving the teacher per session via
-- COALESCE(cs.actual_teacher_id, c.teacher_id). Each session becomes its own row
-- with a single-day days_of_week (the report grid already places by iterating
-- days_of_week, so a 1-element array works unchanged). Room is also resolved per
-- session for consistency, and cancelled/rescheduled sessions are excluded.
...
BEGIN
  WITH
  -- 1. Regular classes: one row per ACTUAL session in the week, teacher resolved
  --    per session (substitute > permanent-change > original).
  class_items AS (
    SELECT
