-- Add teacher_image to get_daily_timetable so the dashboard can show teacher avatars.
-- Image source priority: teachers.profile_image, else the teacher's Google login
-- avatar (auth.users metadata, linked via admin_users.teacher_id).
