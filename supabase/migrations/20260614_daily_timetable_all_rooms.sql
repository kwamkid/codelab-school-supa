-- Dashboard timetable: show ALL active rooms of the (selected) branch, not only
-- rooms that happen to have a class that day — so empty rooms are visible and you
-- can tell at a glance which rooms are free vs busy.
--
-- Only the `rooms` part changes: it now selects from the rooms table (filtered by
-- is_active + branch) instead of DISTINCT over the day's events. Everything else
-- (events, teacher resolution via COALESCE(actual_teacher_id, teacher_id),
-- total_sessions) is unchanged from 20260607_daily_timetable_total_sessions.sql.
