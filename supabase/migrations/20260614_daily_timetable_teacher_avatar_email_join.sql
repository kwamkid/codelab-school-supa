-- Fix: dashboard teacher avatars were blank because the Google-avatar lookup
-- joined auth.users on `u.id = au.id` — but admin_users.id ≠ auth.users.id in this
-- project (they're linked by EMAIL, see CLAUDE/memory). That join matched 0 rows,
-- so teacher_image was always empty and the dashboard fell back to initials.
-- This redefines get_daily_timetable joining auth.users by lower(email) instead,
-- in all three event types (class/makeup/trial). Rooms logic (all active rooms of
-- the branch) and everything else are unchanged from the prior migration.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- ALL active rooms of the (selected) branch — so empty rooms show too.
